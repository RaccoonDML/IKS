...
class SAN(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        # todo : add IKS　in body
        self.gamma = nn.Parameter(torch.zeros(1))
        self.n_resgroups = n_resgroups
        if args.sparseMode in ['lw','fw','kw','pw']:
            self.RG = nn.ModuleList([LSRAG(common.IKS_conv, n_feats, kernel_size, reduction, \
                                           act=act, res_scale=args.res_scale, n_resblocks=n_resblocks) for _ in
                                     range(n_resgroups)])
        elif args.sparseMode=='group':
            self.RG = nn.ModuleList([LSRAG(common.group_conv, n_feats, kernel_size, reduction, \
                                           act=act, res_scale=args.res_scale, n_resblocks=n_resblocks) for _ in
                                     range(n_resgroups)])
        else:
            self.RG = nn.ModuleList([LSRAG(conv, n_feats, kernel_size, reduction, \
